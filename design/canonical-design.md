...
Windows targets Windows 11. Inno Setup 6.4.3 is pinned by compiler hash. Separate architecture installers default to per-user installation, provide Start Menu and optional desktop shortcuts, and register Margin's `.md` and `.markdown` capabilities. Setup offers an unchecked option to open Margin's Windows Default Apps page after installation, and Margin exposes the same supported system picker later. Windows records the user's explicit choice; installation never silently takes over the default association. Upgrades and ordinary uninstall preserve settings and never remove user Markdown files. A shared application mutex prevents replacing/uninstalling a running application; the installer does not force-close unsaved work.
